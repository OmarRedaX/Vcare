...
related: [prd, landscape, glossary, adr-0001-two-service-split]
---
...
|---|---|---|---|
| Users — email, phone, password hash, role, email-verified timestamp | identity-service | access-token claims (`sub`, `role`, `ev`); admin `GET /api/users` | email and phone never leave Identity through `/internal/*`; password hash never leaves Identity at all |
...
| Account status (`pending`, `active`, `suspended`, `rejected`) | identity-service | token `status` claim; `GET /internal/users?ids=` | account status: Identity writes; Care requests changes via Case 1/3 (`PATCH /internal/users/:id/status`) |
...
| Email verifications | identity-service | token `ev` claim | hashed single-use tokens, 24 h |
...
| Verification documents and decisions | care-service | admin application endpoints | stored by object key; served only via signed URLs (≤ 10 min, audited) |
...
| Record attachments | care-service | signed URLs | stored by object key; no delete after lock |
| Help articles | care-service | `GET /api/help-articles` | Phase-2 RAG corpus (read by the AI service through Care's API) |
| Audit log | care-service | `GET /api/audit-logs` (admin, metadata only) | append-only; metadata never holds clinical text or PII |
